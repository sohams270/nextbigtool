...
export default function SubmitPage() {
  ...
  /* auth guard + plan + tool count */
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      if (!user) { router.push("/"); return; }
      setUserId(user.id);

      // Fetch plan and existing tool count in parallel
  ...
  if (!planLoading && userPlan !== "core" && toolCount >= 1) {
    return (
      <>
        <DashTopbar title="Submit Your Tool" subtitle="Upgrade required" />
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", background: "var(--surface-alt)", padding: "40px 24px" }}>
          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 20, padding: "48px 40px", maxWidth: 460, width: "100%", textAlign: "center" }}>
            <div style={{ width: 56, height: 56, borderRadius: "50%", background: "rgba(255,107,53,0.1)", border: "1.5px solid rgba(255,107,53,0.25)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px" }}>
            ...
            <p style={{ fontSize: 13, color: "var(--ink-muted)", lineHeight: 1.65, marginBottom: 28, margin: "0 0 28px" }}>
