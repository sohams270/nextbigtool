...
export default function EditPressReleasePage() {
  ...
  // Fetch press release on mount
  useEffect(() => {
    async function loadRelease() {
      try {
        const res = await fetch(`/api/cms/press-releases/${id}`);
        if (!res.ok) { setError("Failed to load press release"); return; }
        const data = await res.json();
        const pr = data.pressRelease;

        setTitle(pr.title ?? "");
        setStatus(pr.status ?? "draft");
        setAuthor(pr.author ?? "The NBT Team");
        setExcerpt(pr.excerpt ?? "");
        setFeatured(pr.featured ?? false);
        setCompanyName(pr.company_name ?? "");
        setCompanyUrl(pr.company_url ?? "");
        setContactName(pr.contact_name ?? "");
        setContactEmail(pr.contact_email ?? "");
        setTags(pr.tags ?? []);
        setSeoTitle(pr.seo_title ?? "");
        setSeoDescription(pr.seo_description ?? "");
        setSeoSlug(pr.slug ?? "");
        setSeoIndex(pr.seo_index ?? true);

        if (pr.featured_image_url) {
          setFeaturedImage(pr.featured_image_url);
          setFeaturedImageUrl(pr.featured_image_url);
        }

        if (pr.publish_date) {
          const d = new Date(pr.publish_date);
          const pad = (n: number) => String(n).padStart(2, "0");
          const local = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}`;
          setPublishDate(local);
        }

        setInitialContent(pr.content ?? "");
      } catch {
        setError("Network error loading press release");
      } finally {
        setLoading(false);
      }
    }
